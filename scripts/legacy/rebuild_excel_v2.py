...
backup_path = EXCEL_PATH.replace('.xlsx', '_rebuild_bak.xlsx')
...
for row_idx in range(2, len(all_rows) + 1):
    if all_rows[row_idx - 1][16] == 'REVISAR':
        prov = all_rows[row_idx - 1][3]
        detail = all_rows[row_idx - 1][7]
        all_rows[row_idx - 1][16] = get_cat(prov, detail)
        count += 1

print(f'Categorized {count} invoices. Writing new file...')
...
# Escribir datos Facturas
for row_idx, row_data in enumerate(all_rows, 1):
    for col_idx, value in enumerate(row_data, 1):
        new_ws.cell(row_idx, col_idx).value = value

# Copiar otras hojas
src_wb2 = load_workbook(backup_path)
for sheet_name in src_wb2.sheetnames:
    if sheet_name != 'Facturas':
        new_sheet = new_wb.create_sheet(sheet_name)
        src_sheet = src_wb2[sheet_name]
        for row_idx, row in enumerate(src_sheet.iter_rows(), 1):
            for col_idx, cell in enumerate(row, 1):
                new_sheet.cell(row_idx, col_idx).value = cell.value

src_wb2.close()

print('Saving to EXCEL_PATH...')
